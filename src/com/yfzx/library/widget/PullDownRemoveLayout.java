...
public class PullDownRemoveLayout extends FrameLayout {
	private static final String TAG = PullDownRemoveLayout.class
			.getSimpleName();
	...
	public PullDownRemoveLayout(Context context, AttributeSet attrs,
		...
		mShadowDrawable = getResources().getDrawable(
				R.drawable.view_swipeback_shadow_left);
	}
	...
	private void scrollOrigin() {
		if (isEnablePull) {
			...
			mScroller.startScroll(0, mContentView.getScrollX(), 0, -delta,
					Math.abs(delta));
			postInvalidate();
		}
	}
	...
	public void computeScroll() {
		...
		if (isEnablePull) {
			if (mScroller.computeScrollOffset()) {
				mContentView.scrollTo(mScroller.getCurrX(),
						mScroller.getCurrY());
				postInvalidate();
