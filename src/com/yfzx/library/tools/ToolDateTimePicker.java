...
public class ToolDateTimePicker extends ToolDateTime {
	 ...
	 *            0：日期+时间 ；1：日期；2：时间
	 */
	public ToolDateTimePicker(Context context, TextView targetView,
			int chooseType) {
	...
	public void show() {
			...
			new DatePickerDialog(context,
					new DatePickerDialog.OnDateSetListener() {
						public void onDateSet(DatePicker datePicker, int year,
							...
							targetView.setText(formatDateTime(
									calendar.getTime(), DF_YYYY_MM_DD));
						}
		...
		default:
			LinearLayout dateTimeLayout = (LinearLayout) LayoutInflater.from(
					context).inflate(ToolResource.getLayoutId("view_datetime"),
					null);
			datePicker = (DatePicker) dateTimeLayout.findViewById(ToolResource
					.getIdId("datepicker"));
			timePicker = (TimePicker) dateTimeLayout.findViewById(ToolResource
					.getIdId("timepicker"));
	...
	public void show(final DateCallBack dateCallBack) {
		Calendar c = Calendar.getInstance();
		switch (chooseType) {
		case DATE:
			new DatePickerDialog(context,
					new DatePickerDialog.OnDateSetListener() {
						public void onDateSet(DatePicker datePicker, int year,
								int monthOfYear, int dayOfMonth) {
							dateCallBack.onDate(year + "-" + (monthOfYear + 1)
									+ "-" + dayOfMonth);
							targetView.setText(year + "-" + (monthOfYear + 1)
									+ "-" + dayOfMonth);
						}
					}, c.get(Calendar.YEAR), c.get(Calendar.MONTH),
					c.get(Calendar.DATE)).show();
			...
			new TimePickerDialog(context,
					new TimePickerDialog.OnTimeSetListener() {
						public void onTimeSet(TimePicker timePicker,
								int hourOfDay, int minute) {
							Calendar calendar = Calendar.getInstance();
							dateCallBack.onDate(formatDateTime(
									calendar.getTime(), DF_YYYY_MM_DD));
							targetView.setText(formatDateTime(
									calendar.getTime(), DF_YYYY_MM_DD));
						}
	 ...
	 * @return
	 */
	private String initDateTimePicker(DatePicker datePicker,
	...
	private OnDateChangedListener mOnDateChangedListener = new OnDateChangedListener() {
		...
		public void onDateChanged(DatePicker view, int year, int monthOfYear,
			...
			calendar.set(datePicker.getYear(), datePicker.getMonth(),
					...
					timePicker.getCurrentMinute());
			nowDateTime = formatDateTime(calendar.getTime(),
